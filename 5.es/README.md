...
默认分词器是 Standard Analyzer，但是对中文分词效果不友好，拆分为单个字符，导致搜索结果不准确。
...
在 es 的容器里，执行命令安装 ik 分词器插件。使用 Dockerfile 构建 es 镜像
...
BM25 是一种基于概率的模型，用于计算文档与查询之间的相似度。

BM25 算法是公平、高效、稳定的关键词排序算法，ES 默认用它，后面 RAG 做关键词检索也是依赖这个。